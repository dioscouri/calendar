...
class CalendarController extends DSCControllerAdmin
{
 ...
 /*	function display($cachable=false)
	{
		...
		$dispatcher->trigger('onBeforeDisplayAdminComponentCalendar', array() );
		...
		$dispatcher = JDispatcher::getInstance();
		$dispatcher->trigger('onAfterDisplayAdminComponentCalendar', array() );
 ...
 /*	function view()
	 ...
	 /* function release()
	{
		...
		$redirect = "index.php?option=com_calendar&controller=".$this->get('suffix')."&view=".$this->get('suffix')."&task=view&id=".$model->getId()."&donotcheckout=1";
 ...
 /*	function cancel()
	{
		if (!isset($this->redirect)) {
			$this->redirect = 'index.php?option=com_calendar&view='.$this->get('suffix');
		}
 ...
 /*	function delete()
	{
		...
		if (!isset($this->redirect)) {
			...
			: 'index.php?option=com_calendar&view='.$this->get('suffix');
 ...
 /*	function pagetooltip_switch()
	{
		...
		$msg->link 		= 'index.php?option=com_calendar&view='.$view;
		...
		JTable::addIncludePath( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_calendar'.DS.'tables'.DS );
		unset($table);
	...
	/*function footer()
	{
	    Calendar::load( "CalendarModelDashboard", "models.dashboard" );
		$model	= new CalendarModelDashboard();
	 ...
	 * Usage: index.php?option=com_calendar&task=doTask&element=pluginname&elementTask=pluginfunction
	 *
 ...
 /*	function doTask()
	 ...
	 * Usage: index.php?option=com_calendar&task=doTaskAjax&element=pluginname&elementTask=pluginfunction
	 *
