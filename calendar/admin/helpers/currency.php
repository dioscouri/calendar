...
class CalendarHelperCurrency extends CalendarHelperBase 
{
    /**
    ...
    function _($amount, $currency='', $options='')
    {
        ...
        $session_currency = CalendarHelperBase::getSessionVariable( 'currency_id', 0 );
        if( $session_currency )
        {
        ...
        elseif (!empty($currency) && is_numeric($currency))
        {
            ...
            $table = JTable::getInstance('Currencies', 'CalendarTable');
        ...
        if (!empty($convertTo))
        {
            ...
            $amount = CalendarHelperCurrency::convert($table->currency_code, $convertTo, $amount);
        }
    ...
    function convert( $currencyFrom, $currencyTo='USD', $amount='1', $refresh=false )
    {
        ...
        if (empty($rates[$currencyFrom][$currencyTo]))
        {
            ...
            $rates[$currencyFrom][$currencyTo] = CalendarHelperCurrency::getExchangeRate( $currencyFrom, $currencyTo, $refresh );
        }
    ...
    function getExchangeRate( $currencyFrom, $currencyTo='USD', $refresh=false )
    {
        ...
        {
            // get from DB table
            $tableFrom = JTable::getInstance('Currencies', 'CalendarTable');
            ...
            if (!empty($tableFrom->currency_id))
            {
            	...
            	if(Calendar::getInstance()->get('currency_exchange_autoupdate', 1))
            	{
	                ...
	                if ($tableFrom->updated_date < $expire_datetime || $refresh)
	                {
	                    ...
	                    {
	                        $tableFrom->exchange_rate = CalendarHelperCurrency::getExchangeRateYahoo( $currencyFrom, $currencyTo );    
	                    }
        ...
        else
        {
        	// get from DB table
            $tableFrom = JTable::getInstance('Currencies', 'CalendarTable');
            $tableTo = JTable::getInstance('Currencies', 'CalendarTable');
            $tableFrom->load( array('currency_code'=>$currencyFrom) );
