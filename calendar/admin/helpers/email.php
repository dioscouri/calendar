...
if (!class_exists('CalendarHelperBase')) {
    JLoader::register( "CalendarHelperBase", JPATH_ADMINISTRATOR.DS."components".DS."com_calendar".DS."helpers".DS."_base.php" );
}

class CalendarHelperEmail extends CalendarHelperBase
{
    /**
     * Protected! Use getInstance()
     */ 
    protected function CalendarHelperEmail() 
    {
        parent::__construct();
        $this->use_html = true;
    }
    ...
    private function getEmailRecipients( $id, $type = 'order' ) 
    {
        ...
        switch ($type)
        {
            ...
            case 'order':
            default:                
                $model = Calendar::getClass('CalendarModelOrders', 'models.orders');
                $model->setId( $id );
                $order = $model->getItem();
    ...
    private function getEmailContent( $data, $type = 'order' ) 
    {
        ...
        switch ($type) 
        {
            ...
            case "subscription":
                ...
                $link = JURI::root()."index.php?option=com_calendar&view=orders&task=view&id=".$data->order_id;
                ...
                if ( count($data->history) == 1 )
                {
                    ...
                    // get the order body
                    Calendar::load( 'CalendarHelperOrder', 'helpers.order' );
                    $text .= CalendarHelperOrder::getOrderHtmlForEmail( $data->order_id );
                }
                    else
                {
                    // Status Change
