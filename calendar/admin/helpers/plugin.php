...
class CalendarHelperPlugin extends CalendarHelperBase
{
	/**
	 ...
	 */
	function getPluginsWithEvent( $eventName, $folder='Calendar' )
	{
		$return = array();
		if ($plugins = CalendarHelperPlugin::getPlugins( $folder ))
		{
			foreach ($plugins as $plugin)
			{
				if (CalendarHelperPlugin::hasEvent( $plugin, $eventName ))
				{
					$return[] = $plugin;
				}
			}
	 ...
	 * @return array
	 */
	function getPlugins( $folder='Calendar' )
	{
	...
	function hasEvent( $element, $eventName )
	{
	   ...
	   if (!$element || !is_object($element)) {
		...
		$import 	= JPluginHelper::importPlugin( strtolower('Calendar'), $element->element );
		$dispatcher	= JDispatcher::getInstance();
