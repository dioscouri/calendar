...
class CalendarHelperUser extends CalendarHelperBase
{
    ...
    function getPrimaryAddress( $userid, $type='billing' )
    {
        ...
        $model = JModel::getInstance( 'Addresses', 'CalendarModel' );
        switch($type)
        {
            case "shipping":
    ...
    {
        Calendar::load( 'CalendarHelperShipping', 'helpers.shipping' );
        
        $address = CalendarHelperUser::getPrimaryAddress( $userid, 'billing' );
        if (empty($address->zone_id))
        {
            return array();
        }
        
        $geozones = CalendarHelperShipping::getGeoZones( $address->zone_id, '1', $address->postal_code );
        return $geozones;
    }
    ...
    function _sendMail( &$user, $details, $useractivation, $guest=false ) 
    {
        ...
        $lang->load('com_calendar', JPATH_ADMINISTRATOR);
        ...
        $success = CalendarHelperUser::_doMail($mailfrom, $fromname, $email, $subject, $message);
    ...
    {
        // get the order
        $model = JModel::getInstance( 'Orders', 'CalendarModel' );
        ...
        {
            $model = JModel::getInstance( 'Products', 'CalendarModel' );
    ...
    function getUserGroup( $user_id='' )
    {
    	...
    	if (!empty($user_id))
    	{
	    	// get the usergroup
	    	JTable::addIncludePath( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_calendar'.DS.'tables' );
	        $table = JTable::getInstance( 'UserGroups', 'CalendarTable' );
