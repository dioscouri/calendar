...
// $Id: disqus.php,v 1.1.2.3 2009/07/09 20:56:02 robloach Exp $

$disqus_vars = array();
$disqus_vars['user_api_key'] = "zsKOHkIAb9iWvruGqOgDovagixiZX43gqPieeB9jisVleZ961M077xI9uSJ8zps4";
$disqus_vars['forum_id'] = "bmwguggenheimlab";
$disqus_vars['since'] = date("Y-m-d\TH:i", mktime(0, 0, 0, date("m")-1, date("d"),   date("Y")));
 ...
 *       echo $forum->name;
 *     }
 *   } catch(CalendarDisqusException $exception) {
 *     // Display the error.
 *     echo $exception->getMessage();
 *   }
 * @endcode
 */
class CalendarDisqus {
   ...
   *           disqus.com URLâs relating to this thread. Composed of
   ...
   * @param $author_name
   *   the post creatorâs name
   * @param $author_email
   *   the post creatorâs email address
   * @param $options
   *   - partner_api_key
   *   - created_at: Format: 2009-03-30T15:41, Timezone: UTC
   *   - ip_address: the authorâs IP address
  ...
  function call($function, $arguments = array(), $post = FALSE) {
    ...
    if ($info['http_code'] == 200) {
      ...
      if ($disqus->succeeded) {
        ...
        //throw new CalendarDisqusException(isset($disqus->message) ? $disqus->message : NULL, 0, $info, $disqus);
      }
    }
    else {
      //throw new CalendarDisqusException('There was an error querying the Disqus API.', $info['http_code'], $info);
    }
  }
}

/**
 * Any unsucessful result that's created by Disqus API will generate a CalendarDisqusException.
 */
class CalendarDisqusException extends Exception {
  /**
  ...
  /**
   * Creates a CalendarDisqusException.
   * @param $message
