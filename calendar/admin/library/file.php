...
class CalendarFile extends JObject 
{
	/**
	...
	function getDirectory( $media='images' )
	{
		...
		{
			$this->_directory = Calendar::getPath( $media );
		}
		return $this->_directory;
	}
	...
	function handleUpload ($fieldname='userfile') 
	{
		...
		$this->proper_name = CalendarFile::getProperName($userFileName);
    ...
    function readfileChunked($filename, $chunksize = 1024, $retbytes = true)
    {
        ...
        if ($handle === false) {
            JError::raiseWarning( 1, 'CalendarFile::readfileChunked: '.JText::_('Unable to open file') . ": '$filename'");
            return false;
        }
	...
	function getArrayListStorageMethods() 
	{
		...
		if (!is_array($instance)) {
			$instance = array();
			$data = &CalendarFile::getStorageMethods();
