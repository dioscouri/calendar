...
class CalendarImage extends CalendarFile 
{
	var $image;
	var $type;
	...
	function CalendarImage($filename = "") 
	{
		parent::__construct();
	...
	function save($filename, $compression=75, $permissions=null) 
	{
        ...
        if( $image_type == IMAGETYPE_JPEG ) {
            ...
            {
                $this->setError( "CalendarImage::save( 'jpeg' ) Failed" );
            }
        } elseif( $image_type == IMAGETYPE_GIF ) {
            if (!$success = imagegif($this->image, null ))
            {
                $this->setError( "CalendarImage::save( 'gif' ) Failed" );
            }
        } elseif( $image_type == IMAGETYPE_PNG ) {
            if (!$success = imagepng($this->image, null ))
            {
                $this->setError( "CalendarImage::save( 'png' ) Failed" );
            }
        }
	...
	function resize( $width, $height ) 
	{
        ...
        //		    imagepng($new_image);
        //				
        //		imagecopyrecalendard($new_image, $this->image, 0, 0, 0, 0, $width, $height, $this->getWidth(), $this->getHeight());
        //		
        //		imagefill($new_image, 0, 0, $trans_colour);
        //		
