...
class CalendarMenu extends JObject
{
    private $_name = array();
    private $_menu;
    ...
    function __construct($name = 'submenu')
    {
        ...
        $initialpath = 'media' . DS . 'com_calendar' . DS . 'menus';
     ...
     * @return mixed
     * 
     * Returns a reference to a CalendarMenu object or false if submenus have been disabled by an admin
     */
    function & getInstance($name = 'submenu')
    {
        ...
        if (empty ($instances[$name])) {
            $instances[$name] = new CalendarMenu($name);
        }
	...
	public static function treerecurse($id, $indent, $list, &$children, $maxlevel=9999, $level=0, $type=1, $pre=null, $spacer=null)
	{
		if (@$children[$id] && $level <= $maxlevel)
		{
			foreach ($children[$id] as $v)
			{
				...
				$list = CalendarMenu::treerecurse($id, $indent . $spacer, $list, $children, $maxlevel, $level+1, $type, $pre, $spacer);
			}
		}
		return $list;
	}
