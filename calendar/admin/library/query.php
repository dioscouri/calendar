 ...
 * @since		1.6
 */
class CalendarQueryElement extends JObject
{
 ...
 * @since		1.6
 */
class CalendarQuery extends JObject
{
	...
	public function select($columns)
	{
		...
		if (is_null($this->_select)) {
			$this->_select = new CalendarQueryElement('SELECT', $columns);
		} else {
    ...
    {
        $this->_type = 'delete';
        $this->_delete = new CalendarQueryElement('DELETE', array(), '');
        return $this;
    }
    ...
    {
        $this->_type = 'insert';
        $this->_insert = new CalendarQueryElement('INSERT INTO', $tables);
        return $this;
    }
    ...
    {
        $this->_type = 'update';
        $this->_update = new CalendarQueryElement('UPDATE', $tables);
        return $this;
    }
	...
	public function from($tables)
	{
		if (is_null($this->_from)) {
			$this->_from = new CalendarQueryElement('FROM', $tables);
		} else {
	...
	public function join($type, $conditions)
	{
		...
		$this->_join[] = new CalendarQueryElement(strtoupper($type) . ' JOIN', $conditions);
    ...
    public function set($conditions, $glue=',')
    {
        if (is_null($this->_set)) {
            ...
            $this->_set = new CalendarQueryElement('SET', $conditions, "\n\t$glue ");
        } else {
	...
	public function where($conditions, $glue='AND')
	{
		if (is_null($this->_where)) {
			$glue = strtoupper($glue);
			$this->_where = new CalendarQueryElement('WHERE', $conditions, "\n\t$glue ");
		} else {
	...
	public function group($columns)
	{
		if (is_null($this->_group)) {
			$this->_group = new CalendarQueryElement('GROUP BY', $columns);
		} else {
	...
	public function having($conditions, $glue='AND')
	{
		if (is_null($this->_having)) {
			$glue = strtoupper($glue);
			$this->_having = new CalendarQueryElement('HAVING', $conditions, "\n\t$glue ");
		} else {
	...
	public function order($columns)
	{
		if (is_null($this->_order)) {
			$this->_order = new CalendarQueryElement('ORDER BY', $columns);
		} else {
