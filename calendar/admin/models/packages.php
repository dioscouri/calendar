...
class CalendarModelPackages extends CalendarModelBase
{
	//public $cache_enabled = false;
	public $cache_lifetime = '2592000'; 
	
    function __construct($config = array())
    {
        parent::__construct($config);
        $this->setState( 'select', array('package.*', 'series.*', 'items.*') );
	...
	protected function _buildQueryFields( &$query )
	{
	    ...
	    if (empty($fields)) {
	        $this->setState( 'select', array('package.*', 'series.*') );
	        $fields = array();
	    }
	...
	protected function _buildQuery( $refresh=false )
	{
	    ...
	    $query = \MappableQuery\MappableQuery::factory('JALC\Queries\PackageQuery');
	...
	protected function prepareItem( &$item, $key=0, $refresh=false )
	{
	    ...
	    $item->link = 'index.php?option=com_calendar&view=packages&task=edit&id=' . $item->getDataSourceID();
	    $item->checked_out = false;
	    $item->series_title = @$item->series->title;
	    
	    // fix this -- need to get back the expected PackageEntity, but only getting back stdClass
	    $this->cacheItem( $item, $item->getDataSourceID() );
	}
	...
	public function setJoomlaProperties( &$item, $refresh=false )
	{
	    ...
	    if (empty($joomla_properties[$cache_key]) || $refresh)
	    {
	        ...
	        if (!$cached_item || $refresh)
	        {
	            ...
	            if (!empty($cached_item->package_id)) {
	...
	public function getSurrounding( $pk=null )
	{
	    ...
	    {
            $row = $list[$i];
            $item_id = $item->getDataSourceID();
            $row_id = $row->getDataSourceID();
            if ($row_id == $item_id)
            {
                $found = true;
                $prev_num = $i - 1;
	...
	public function getGroups($season=null, $refresh=false)
	{		
	    if (empty($season)) 
	    {
	        $season = JALC\Entities\Season::getCurrentFiscalYear();
	    }
	    
	    if (empty($this->_groups[$season]) || $refresh)
	    {
	        ...
	        $cache_key = base64_encode(serialize($season)) . '.groups';
	         
	        $classname = strtolower( get_class($this) );
	        $cache = JFactory::getCache( $classname . '.groups', '' );
	        $cache->setCaching($this->cache_enabled);
	        $cache->setLifeTime($this->cache_lifetime);
	        $list = $cache->get($cache_key);
	        if (!$list || $refresh)
	        {
	            ...
	            $this->setState('select', array('package.*', 'series.*', 'items.*'));
	            $this->setState('filter_season', $season);
	            $query = $this->getQuery(true);
	            $query->find( array( 'isEducationPackage' => false ) );
	            $packages = $query->fetchObjects();
	            ...
	            foreach ( $packages as $key=>$item )
	            {
	                ...
	                $series_key = JFilterOutput::stringURLSafe( $item->series_title );

	                if (!empty($series_key)) 
	                {
	                    if (empty($list[$series_key])) {
	                        $list[$series_key] = new stdClass();
	                        $list[$series_key]->title = $item->series_title;
	                        $list[$series_key]->alias = $series_key;
	                        $list[$series_key]->ordering = '9999';
	                        $list[$series_key]->full_image = null;
	                        $list[$series_key]->purchase_url = null;
	                        $list[$series_key]->price_range = null;
	                        $list[$series_key]->short_description = null;
	                        $list[$series_key]->long_description = null;
	                        $list[$series_key]->items = array();
	                        $list[$series_key]->shows = array();
	                        $list[$series_key]->performances = array();
	                    }
	                    
                        $list[$series_key]->items[] = $item;
                        if ($item->ordering < $list[$series_key]->ordering) {
                            $list[$series_key]->ordering = $item->ordering; 
                        }
                        
                        if (empty($list[$series_key]->full_image) && !empty($item->full_image)) {
                            $list[$series_key]->full_image = $item->full_image;
                        }
                        
                        if (empty($list[$series_key]->short_description) && !empty($item->shortDescription)) {
                            $list[$series_key]->short_description = $item->shortDescription;
                        }
                        
                        if (empty($list[$series_key]->long_description) && !empty($item->fullDescription)) {
                        	$list[$series_key]->long_description = $item->fullDescription;
                        }
                        
                        foreach ($item->items as $performance) 
                        {
                            $show = $performance->show;
                            $model->setJoomlaProperties( $show );
                            ...
                            $show->date_range = $date_range;
                            
                            if (empty($list[$series_key]->shows[$show->getDataSourceID()])) {
                                $list[$series_key]->shows[$show->getDataSourceID()] = $show;
                            }
                            if (empty($list[$series_key]->performances[$performance->getDataSourceID()])) {
                                $list[$series_key]->performances[$performance->getDataSourceID()] = $performance;
                            }
                        }
                        
                        if (empty($list[$series_key]->purchase_url) && $item->getPurchaseUrl()) {
                        	$list[$series_key]->purchase_url = $item->getPurchaseUrl();
                        }
                        
                        if (empty($list[$series_key]->price_range) && $item->displayPrices) {
                        	$list[$series_key]->price_range = $item->displayPrices;
                        }
	                } 
	            }
	            
	            jimport( 'joomla.utilities.arrayhelper' );
	            JArrayHelper::sortObjects($list, 'ordering');
	            foreach ($list as $key=>$item) 
	            {
	                JArrayHelper::sortObjects($item->items, 'ordering');
	        ...
	        $this->_groups[$season] = $list;
	    }
	    
	    return $this->_groups[$season];
	}
	...
	public function clearCache()
	{
	    ...
	    $classname = strtolower( get_class($this) );
	    parent::cleanCache($classname . '.groups');
	}
}
