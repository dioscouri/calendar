...
//Calendar::load( 'CalendarQuery', 'library.query' );

class CalendarTable extends DSCTable
{
	/**
	 * constructor
	 */
...
/*	function store( $updateNulls=false )
	...
	/*function filterHTML( $fieldname='description' )
	{
		...
		if (is_array($filterGroups) && in_array( $gid, $filterGroups ))
		{
			...
			$filterCalendar		= preg_split( '#[,\s]+#', trim( $config->get( 'filter_calendar' ) ) );
			...
			switch ($filterType)
			{
				case 'NH':
					...
					break;
				case 'WL':
					$filter	= new JFilterInput( $filterCalendar, $filterAttrs, 0, 0 );
					break;
				case 'BL':
				default:
					$filter	= new JFilterInput( $filterCalendar, $filterAttrs, 1, 1 );
					break;
			}
