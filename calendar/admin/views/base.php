...
class CalendarViewBase extends DSCViewAdmin
{
	...
	/*function displayTitle( $text = '' )
	{
		...
		JToolBarHelper::title( $title, Calendar::getName() );
	}
    	...
    	echo "<table width='100%'>";
    		echo "<tr>";
	    		echo "<td style='width: 180px; padding-right: 5px; vertical-align: top;' >";
					...
					if ($menu =& CalendarMenu::getInstance($menuname)) {
 ...
 /*	function _default($tpl='')
	{
		...
		// form
			...
			$action = $this->get( '_action', "index.php?option=com_calendar&view={$view}" );
			$form['action'] = $action;
 ...
 /*	function _form($tpl='')
	{
		...
		// form
			...
			$controller = strtolower( $this->get( '_controller', JRequest::getVar('controller', JRequest::getVar('view') ) ) );
			$view = strtolower( $this->get( '_view', JRequest::getVar('view') ) );
			$action = $this->get( '_action', "index.php?option=com_calendar&view={$view}&task=edit&id=".$model->getId() );
			$validation_url = $this->get( '_validation', "index.php?option=com_calendar&view={$view}&task=validate&format=raw" ); 
