...
class CalendarViewElementArticle extends JView
{
	function display( )
	{
		...
		$link = 'index.php?option=' . 'com_calendar' . '&task=elementArticle&tmpl=component&object=' . $object;
?>
		<form action="<?php echo $link; ?>" method="post" name="adminForm" id="adminForm">
				...
				<tr>
					<td width="100%">
						...
						<button onclick="this.form.submit();"><?php echo JText::_( 'Go' );
															  ?></button>
						<button onclick="getElementById('search').value='';this.form.submit();"><?php echo JText::_( 'Reset' );
																								?></button>
					</td>
					<td nowrap="nowrap">
						<?php
			...
			?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td>
						<a style="cursor: pointer;" onclick="window.parent.jSelectArticle('<?php echo $row->id;
