...
<div class="wrap <?php echo htmlspecialchars($params->get('moduleclass_sfx')); ?>">
    ...
    <h2 class="see-all">
        <a href="<?php echo JRoute::_( "index.php?option=com_calendar" . $itemid_string . "&reset=1" ); ?>">
...
if (!empty($items->today_items)) 
{
    ...
    ?>
        ...
        <ul class="slides events-list list">
        <?php 
        foreach ($items->today_items as $key=>$item) 
        {
            ?>
            <li class="slide wrap eventinstance instance" id="<?php echo $item->getDataSourceID(); ?>">
                    <div class="image-frame wrap" data-position="<?php echo $key; ?>">
...
if (!empty($items->this_week_items))
{
    ...
    ?>
        ...
        <ul class="slides events-list list need-actionbuttons">
        <?php
...
<?php if (count($items->today_items) > '2') { ?>
...
jQuery(document).ready(function() {
    jQuery('.today_items').carousel({
        ...
        duration: <?php echo $params->get( 'slideshow_duration', '300' ); ?>,
        ...
        containerHeight: <?php echo $params->get( 'slideshow_container_height', '233' ); ?>,
        slideWidth: <?php echo $params->get( 'slideshow_slide_width', '610' ); ?>,
        slideHeight: <?php echo $params->get( 'slideshow_slide_height', '98' ); ?>
    });

    jQuery('.today_items').find('.image-frame').each(function(){
        if (jQuery(this).parents('.slide').hasClass('active')) {
        	jQuery(this).parents('.slide').addClass('highlighted');
            jQuery(this).fadeIn();
        } else {
            jQuery(this).fadeOut();
        }
    ...
    jQuery('.today_items').find('.slide').mouseenter(function(){
    	jQuery('.today_items').find('.highlighted').removeClass('highlighted');
    	jQuery('.today_items').find('.active').removeClass('active');
    	jQuery(this).addClass('highlighted');
        jQuery(this).find('.image-frame:hidden').show();
        var p = jQuery(this).attr('data-position');
        jQuery('.today_items').find('.image-frame:visible').each(function(){
            if (p != jQuery(this).attr('data-position')) {
                jQuery(this).hide();
            }
        });        
    });
...
jQuery(document).ready(function() {
    ...
    jQuery('.this_week_items').find('.image-frame').each(function(){
        if (jQuery(this).parents('.slide').hasClass('active')) {
        	jQuery(this).parents('.slide').addClass('highlighted');
            jQuery(this).fadeIn();
        } else {
    ...
    jQuery('.this_week_items').find('.slide').mouseenter(function(){
    	jQuery('.this_week_items').find('.highlighted').removeClass('highlighted');
    	jQuery('.this_week_items').find('.active').removeClass('active');
    	jQuery(this).addClass('highlighted');
        jQuery(this).find('.image-frame:hidden').show();
        var p = jQuery(this).attr('data-position');
        jQuery('.this_week_items').find('.image-frame:visible').each(function(){
            if (p != jQuery(this).attr('data-position')) {
                jQuery(this).hide();
            }
        });        
...
<script type="text/javascript">
Jalc.displayActiveImage = function(slideshow) {
	jQuery('.this_week_items').find('.highlighted').removeClass('highlighted');
	jQuery('.today_items').find('.highlighted').removeClass('highlighted');
    slideshow.slides.find('.image-frame').each(function(){
        if (jQuery(this).parents('.slide').hasClass('active')) {
        	jQuery(this).parents('.slide').addClass('highlighted');
