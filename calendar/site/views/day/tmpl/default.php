...
<div id="calendar-content" class="wrap day-view">

    <h2 class="content-title events-date-range left">
        <?php echo date( 'M j, Y', strtotime( $state->filter_date_from) ); ?>    
    </h2>
    
    <div id="date-navigation" class="wrap right">
        <ul class="controls controls-medium flat">
    		<li class="prev"><a href="<?php echo JRoute::_( 'index.php?option=com_calendar&view=day&date=' . $this->date_navigation->prev . $itemid_string ); ?>"><?php echo JText::_( "Previous" ); ?></a></li>
    		<li class="next"><a href="<?php echo JRoute::_( 'index.php?option=com_calendar&view=day&date=' . $this->date_navigation->next . $itemid_string ); ?>"><?php echo JText::_( "Next" ); ?></a></li>
        </ul>    
    </div>
		...
		<?php if ( !empty($this->items) ) { ?>
            ...
            foreach ($this->items as $key=>$item) 
            {
                ...
                <li class="wrap eventinstance instance" id="<?php echo $item->getDataSourceID(); ?>">
                        ...
                        <div class="image-frame left small <?php if (empty($item->event_full_image)) { ?>no-image<?php } ?>">
                        ...
                        <div class="inner wrap left">
                            ...
                            <div class="overview left">
                                <h5>
                                    <?php
                                    $textAboveTitle = '';
                                    //Only show series for education courses
                                    if ($item->isCourseSession && $item->series_title) {
                                        $textAboveTitle .= $item->series_title . " | " ;
                                    }
                                    //Show 'Visiting Presenter' if appropriate
